...
func intro_tutorial_text():
	Global.spawn_penny.emit("Hey-hey-hi! I’m Penny,\nyour digital hacking assistant.")
	Global.spawn_penny.emit("I’m here to make sure ya\nhacking right, and them\ncounterhackers don’t kill you dead")
	Global.spawn_penny.emit("Now, see them two big buttons\non the right, about halfway\ndown called “Add” and “Sub”")
	Global.spawn_penny.emit("As you’d guess, they add and\nSubtract! So, ya select one of ’em \nand then two squares in the\nactive row or column")
	Global.spawn_penny.emit("Bear in mind: ’cos this is a\ncomputer, numbers will overflow if\nthey’re more than 63, and underflow\nif they’re less than 0")
	Global.spawn_penny.emit("Now you have a go!\nTry doin’ an operation!")
	
func operation_done_tutorial_text():
	Global.spawn_penny.emit("Whoah! Nicely done.")
	Global.spawn_penny.emit("See that big bar, right\nat the top? Once ya start hackin’,\nthe antivirus be onta you")
	Global.spawn_penny.emit("But you can throw ’em of\nyour scent, eh? See that big button\nin the top right labelled “restart”?")
	Global.spawn_penny.emit("Well, if they’re about to\ncatch you, whack that button!\nThey’ll lose your scent, but the hard\ndrive you’re hacking won’t reset.")
	Global.spawn_penny.emit("Prob’m is, ya can’t just\ngo restartin’ willy-nilly\nYou’ve gotta fulful the restart objective\n(see that text right near the button?)")
