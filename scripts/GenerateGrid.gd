	...
	match level.goal:
		...
		LevelResource.Goal.MakePointsEqual:
			for point in level.points_to_equalise:
				grid.get_cell(point).set_special_feature(GridElement.GoalSpecialFeature.PointEqualisation, text_size_modifier)
...
func check_win_loss_criteria():
	if counterhack_bar.moves_remaining <= 0:
		if restart_criteria.done():
			...
			get_tree().create_timer(3.0).timeout.connect(func(): Global.go_to_level_select.emit())
	elif is_goal_complete(level.goal):
		Global.spawn_notification.emit("Level complete. Good job, agent.")
		var level_idx = (SceneManager.active_scene as LevelManager).levels.find(level)
		Global.completed_levels.push_back(level_idx + 1)
		get_tree().create_timer(3.0).timeout.connect(func(): Global.go_to_level_select.emit())
