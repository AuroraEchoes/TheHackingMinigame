...
func update_goal_text(level: LevelResource, grid: Grid) -> void:
	...
	match level.goal:
		LevelResource.Goal.UnlockTile:
			...
			text += "Unlock the [color=#ffd000]󰌾 Locked Tile[/color] by setting it to 0 (currently " + str(unlock_tile_val) + ")"
		LevelResource.Goal.MakePointsEqual:
			...
			for point in level.points_to_equalise:
				...
				if point != level.points_to_equalise.back():
					points += ", "
			text += "Set all the [color=#ffd000]󰫤 Parity Tiles[/color] to the same number (currently " + points + ")"
		LevelResource.Goal.SumOfRowCol:
			...
			text += "Make all [color=#ffd000]  Checksum Tiles[/color] in the " + row_col + " sum to " + str(level.sum_of_selected_row_col) + " (currently " + str(sum) + ")"
		LevelResource.Goal.AllOddAllEven:
			...
			for tile in grid.get_row_or_col(level.selected_row_or_col):
				if tile.get_number() % 2 == 0:
					...
					n_odd += 1
			text += "Make all [color=#ffd000]󱔼 Data Corruption Tiles[/color] " + odd_even + " (Currently " + str(n_odd) + " odd, " + str(n_even) + " even)"
